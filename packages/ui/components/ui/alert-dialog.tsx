...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn(
      'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 bg-web3sheet-black fixed inset-0 z-50 opacity-50',
      className,
    )}
    {...props}
    ref={ref}
    ...
    >
      <Module noPadding>
        <ModuleGridHeader keepDesktopHeaderOnMobile className="relative mb-4 flex items-center">
          ...
          <AlertDialogPrimitive.Cancel className="ring-offset-w3s-background focus:ring-w3s-ring data-[state=open]:bg-w3s-secondary absolute right-8 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none">
            <X className="h-6 w-6" />
